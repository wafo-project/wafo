...
*
      ...
      PARAMETER ( N = 20, NI = 5000, NF = 8, LW = 20000 )
      DOUBLE PRECISION VALS(NF), ERRS(NF), WORK(LW)
      PRINT '('' Test for N = '', I2 )', N
      PRINT *, 'F Evals.\\F  1      X       X^2     XY      XY^2',
     &  '     X^4/3   (XY)^2  Test 1 Key'
      DO KEY = 1, 3
         NP = NI
         RS = 0
         DO I = 1, 3
     ...
     &           VALS, ERRS, NV, IN, WORK )
            PRINT '(I8, 1X, 5F8.5,3F9.5,I3)', NV, VALS, KEY
            PRINT '('' Errors  '', 5F8.5,3F9.5)', ERRS
            NP = NP*4
            RS = 1
          END DO
      END DO
