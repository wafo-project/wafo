...
%
% CALL [pval, fstat,df_b, df_e] = anovan (data, grps,options)
% 
% ANOVAN perform a multi-way analysis of variance (ANOVA).  
% The goal is to test whether the population means of data taken from K 
% different groups are all equal.
%
% Data is a single vector DATA with groups specified by
% a corresponding matrix of group labels GRPS, where GRPS
% has the same number of rows as DATA. For example, if
% DATA = [1.1;1.2]; GRPS = [1,2,1; 1,5,2];
...
%
% The p-value (1 minus the CDF of this distribution at FSTAT is
% returned in PVAL.
%
...
%
% Example % Type anovan for more examples
%  % From web site: www.mathworks.com/access/helpdesk/help/toolbox/stats/anova2.html
%      popcorn= [  5.5 4.5 3.5; 5.5 4.5 4.0; 6.0 4.0 3.0;...
%                  6.5 5.0 4.0; 7.0 5.5 5.0; 7.0 5.0 4.5];
%  % Define groups so reps = 3
%      groups = [  1 1;1 2;1 3;1 1;1 2;1 3;1 1;1 2;1 3;...
%                  2 1;2 2;2 3;2 1;2 2;2 3;2 1;2 2;2 3 ];
%      anovan( popcorn(:), groups, 'model', 'full')
%   help anovan>testexamples    % for more examples
%
% See also fitls
...
%
% ANOVAN is free software; you can redistribute it and/or modify it
% under the terms of the GNU General Public License as published by
% the Free Software Foundation; either version 3, or (at your option)
% any later version.
%
% ANOVAN is distributed in the hope that it will be useful, but
...
%     end
    ...
    elseif strcmp(modeltype,'full')
       max_interact  = size(grps,1);
    else
       error('modeltype %s is not supported', modeltype);
    end
    ng = length(grp_map);
...
function int_tbl =interact_tbl(nw, ng, max_interact)
    ...
    idx = ( sum(inter_tbl,2)' > max_interact );
    inter_tbl(idx,:) =[];
...
function    [gn, gs, gss] = raw_sums(data, g, ng, int_tbl)
    nw=    size(g,2);
    ndata= size(g,1);
    [gn,gs, gss]=  deal(zeros((ng+1)^nw, 1));
    for i=1:ndata
        % need offset by one for indexing
        datapt= data(i);
        idx = 1+ int_tbl*g(i,:)';
        gn(idx)  =gn(idx)+1;
        gs(idx)  =gs(idx)+datapt;
        gss(idx) =gss(idx)+datapt^2;
    end
end %function
...
function printout( stats, stats_tbl )
  ...
  fprintf('Source of Variation        Sum Sqr   df      MeanSS    Fval   p-value\n');
  fprintf('*********************************************************************\n');
  ...
  for i= order(:)'
      ...
      fprintf('Factor %15s %10.2f  %4d %10.2f  %7.3f  %7.6f\n', ...
         str, stats(i,:) );
  end
  fprintf('\n');
end %function
