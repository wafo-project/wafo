...
int disufq(double *rvec, double *ivec,
	   double *rA,   double *iA,
	   double *w,    double *kw,
	   double h,     double g,
	   int nmin, int nmax,
...
{ 
  ...
  int i,jy,ix,iz1,iz2,iv1,iv2,ixi,jyi;
...
int disufq2(double *rsvec, double *isvec,
...
{
  double *rvec,*ivec,*rdvec,*idvec,*rA,*iA,*w,*kw,h,g;
  int length,nmax,nmin,n,m;
  ...
  if( !mxIsComplex(prhs[0])) {
    mexErrMsgTxt("Input A  must be complex.\n");
  }
  ...
  nmin = (int) mxGetScalar(prhs[5]);
  nmax = (int) mxGetScalar(prhs[6]);
  ...
  if (nlhs == 1){
    length = disufq(rvec,ivec,rA,iA,w,kw,h,g,nmin,nmax,m,n);
  }
  else{ // Calculate the sum and difference frequency effects separately

    /*  allocate enough memory to hold the resultant matrix */
    plhs[1] = mxCreateDoubleMatrix(m,n, mxCOMPLEX);
  
    /* and Create a pointers to the output matrix */
    rdvec    = mxGetPr(plhs[1]);  
    idvec    = mxGetPi(plhs[1]);  
    /*  call the C subroutine */
   
    length = disufq2(rvec,ivec,rdvec,idvec,rA,iA,w,kw,h,g,nmin,nmax,m,n);
  }
}
