...
%    wxpdf=mvnpdf([v 0],[zeros(Nu,1) uu'],SZZ)*ones(1,Ny);
    ...
    TTfu.f{vv}=max(0,smooth(y,gradient(TTFUP(1:end-1),dx),smoothp,y));
    
    if nout > 0, 
        TTFDO=simpson(uu',wxpdf.*TTF_do);
        TTFDO=TTFDO/TTFDO(end);  
        TTFd.f{vv}=TTFDO(1:end-1);  
        TTfd.f{vv}=max(0,smooth(y,gradient(TTFDO(1:end-1),dx),smoothp,y));
    end
    if nout > 1, 
        STFUP=simpson(uu',wxpdf.*STF_up);
        STFUP=STFUP/STFUP(end);
        STFu.f{vv}=STFUP(1:end-1);
        STfu.f{vv}=max(0,smooth(y,gradient(STFUP(1:end-1),dx),smoothp,y));
    end
    if nout > 2,
        STFDO=simpson(uu',wxpdf.*STF_do);
        STFDO=STFDO/STFDO(end);    
        STFd.f{vv}=STFDO(1:end-1);
        STfd.f{vv}=max(0,smooth(y,gradient(STFDO(1:end-1),dx),smoothp,y));
    end
    if nout > 3,
        VTFUP=simpson(uu',wxpdf.*VTF_up);
        VTFUP=VTFUP/VTFUP(end);
        VTFu.f{vv}=VTFUP(1:end-1);
        VTfu.f{vv}=max(0,smooth(y,gradient(VTFUP(1:end-1),dx),smoothp,y));
    end
    if nout > 4,
