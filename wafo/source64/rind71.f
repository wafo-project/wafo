...
!   
! CALL SETDATA(REPS,EPSS,EPS2,xCutOff,NINT1,MAXPTS) 
!
!                   GLOBALDATA module :
...
!
!CALL RINDD(E,S,m,xc,indI,Blo,Bup,xcScale);
!
...
!*****************************************************
      ...
      END INTERFACE
                                !--------------------------------
      CONTAINS
      SUBROUTINE SETDATA (dREPS,dEPSS,dEPS2,dXc,dNINT,maxpts) 
      USE GLOBALDATA
      USE FIMOD
      ...
      INTEGER, INTENT(in) :: dNINT,maxpts
      INTEGER :: N=1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE RINDD (fxind,Big1, Ex, xc1,indI,Blo,Bup,dxcScale)  
     ...
     &     SQTWOPI1,xCutOff,SCIS,Ntscis,COVix,EPS
	  ...
	  DOUBLE PRECISION, OPTIONAL,      INTENT(in) :: dxcScale
      INTEGER,          DIMENSION(:  ), INTENT(in) :: indI 
! local variables
      INTEGER                          :: J,ix,Ntdcmj,Nst,Nsd,INFORM
      DOUBLE PRECISION                 :: xind,SQ0,xx,fxc,quant,XcScale

      XcScale=0.0
      IF (PRESENT(dxcScale)) THEN
	XcScale = dxcScale
      ENDIF
...
! J. computational Graphical Statistics, Vol.1, pp 141--149
	...
	CASE (4)
     ...
     &     ERROR, VALUE, INFORM )                 !        Call the subregion adaptive integration subroutine  
