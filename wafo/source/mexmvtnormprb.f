...
*
*
* MEXMVTNORMPRB Computes multivariate T probability by Genz' algorithm.
...
*
*  CALL [value,error,inform] = mexmvtnormprb(correl,A,B,nu,delta,
*                                       abseps,releps,maxpoints);
*
...
C
      ...
      integer V_pr, E_pr, IF_pr, C_pr, A_pr, B_pr, D_pr
C-----------------------------------------------------------------------
C
      integer nlhs, nrhs
      ...
      INTEGER Nd, NN, MAXPTS, IFT,NU
      PARAMETER ( Nd = 100, NN = (Nd - 1)*Nd/2)
      PARAMETER (INFINITY = 37.d0)
      DOUBLE PRECISION COV(NN), LOW(Nd), UP(Nd), DELTA(Nd)
      INTEGER INFIN(Nd)
...
C     Column * row should be smaller than 5*990
      ...
      k = mxGetN(prhs(4))*mxGetM(prhs(4))
      if(n.ne.m.or.n.ne.k.or. n*(n-1)/2.ne.size) then
...
C     Create matrix for the return argument.
      ...
      call mxCopyPtrToReal8(D_pr,DELTA,n)
*     
...
*            if INFIN(I) = 2, Ith limits are [LOWER(I), UPPER(I)].		
	...
	RELEPS = mxGetScalar(prhs(7))
	MAXPTS = mxGetScalar(prhs(8))
C	call mxCopyPtrToInteger4(M_pr,MAXPTS,1)

C     Call the computational subroutine.
        CALL MVTDST( N, NU, LOW, UP, INFIN, COV, DELTA, 
