...
%   [CI0,Lp0] = ciproflog(phat, 'i', 2, 'x', x, 'link', @lnkweib, 'plotflag', 1);
%
...
%   [CI2,Lp2] = ciproflog(phat2,'i',2,'x',x,'link',@lnkgenpar,'plotflag',1);
%   [CI3,Lp3] = ciproflog(phat2,'i',2,'logR',-log(9900),'link',@lnkgenpar,'plotflag',1);
%
%   close all;
%
...
% by pab Sep 2007
  ...
  options = struct('i',1,'pmin',[],'pmax',[],'N',100,'x',[],'logR',[],...
                   ...
                   'optimset', optimset('disp','off','TolX',1e-5,...
                   'TolFun',1e-5,'MaxIter',500));
  ...
  if options.check
     logfun1 = @(x)mylogfun(x,phatv(i_fixed),[], phatv, isfree, i_fixed, [], [], data1, logfun, dist, pdfoptions);
    if ~isempty(phatfree)
      phatfree = fminsearch(logfun1,phatfree,options.optimset);
    end
    LLt = -logfun1(phatfree);
    if LLt>LLmax
      foundNewphat = true;
  ...
  if isempty(options.pmin) || isempty(options.pmax)   
    if ~isempty(link)
      if isProfileX
        invfun = @(x) myinvfun(x, phatv, prb, isnotfixed, invfun, optsfun);
        drl = gradest(invfun, phatv(isnotfixed));
      else
        prbfun = @(x) myprbfun(x, phatv, qntl, isnotfixed, prbfun, optsfun);
        drl = gradest(prbfun,phatv(isnotfixed));
      end
  ...
  for ix = k1:-1:1
    logfun1 = @(x) mylogfun(x, pvec(ix),link, phatv, isfree, i_fixed, prb, qntl, data1, logfun, dist, pdfoptions);
    if ~isempty(phatfree)
      phatfree = fminsearch(logfun1, phatfree, options.optimset);
    end
    LL(ix) = -logfun1(phatfree);
  ...
  if options.check
    ...
    phatfree = phatv(isfree);
    logfun1 = @(x) mylogfun(x, phatv(i_fixed), [], phatv, isfree, i_fixed, prb, qntl, data1, logfun, dist, pdfoptions);
    if ~isempty(phatfree)
      phatfree = fminsearch(logfun1,phatfree,options.optimset);
    end
    LLt = -logfun1(phatfree);
...
%% Subfunctions
function L = mylogfun(phatfree1,fix_p,linkfun1, phatv, isfree, i_fixed, prb, qntl, data1, logfun, dist, pdfoptions)
  ...
  L = feval(logfun, mphat,data1,pdfoptions{:},dist);
end % function mylogfun

function x1 = myinvfun(phatnotfixed, phatv, prb, isnotfixed,invfun, optsfun)
  mphat = phatv;
...
function prb1 = myprbfun(phatnotfixed, phatv, qntl, isnotfixed, prbfun, optsfun)
  mphat = phatv;
  mphat(isnotfixed) = phatnotfixed;
  cphat = num2cell(mphat,1);
