...
% 
% Draws a horizontal line on the current axes at the location specified by 'y'.  
% Optional arguments are 'linetype' (default is 'r:') and 'label', which 
% applies a text label to the graph near the line.  The label appears in the 
...
%
% The HandleVisibility property of the line object is set to "off", so not only 
% does it not appear on legends, but it is not findable by using findobj.  
% Specifying an output argument causes the function to return a handle to the 
% line, so it can be manipulated or deleted.  Also, the HandleVisibility can be 
