...
WZ = zeros(Nu,Nv,Ntt);  % Note the order - it is permuted at the end
XZ = zeros(Nu,Nv,Ntt); YZ=zeros(Nu,Nv,Ntt);
W2Z = zeros(Nu,Nv,Ntt); X2Z=zeros(Nu,Nv,Ntt); Y2Z=zeros(Nu,Nv,Ntt);
X2drift=zeros(Nu,Nv,Ntt); Y2drift=zeros(Nu,Nv,Ntt);
...
[x_eta2r,x_eta2i,coshkz,sinhkz,coshk,sinhk,expk,cost,sint] =...
    partkerng(x_w, x_k, x_e*0, theta, cosd, h, [], z) ;
x_temp = -i*(expk.*(coshkz./sinhk)); % MP notation
...
parfor ku=1:Nu,
    [x_eta2r,x_eta2i,coshkz,sinhkz,coshk,sinhk,expk,cost,sint] =...
        partkerng(x_w, x_k, x_e*0, theta, cosd, h, [], z) ;
    WZZ = zeros(Nv,Ntt); XZZ=WZZ; YZZ=WZZ;
    WZZ2 = zeros(Nv,Ntt); XZZ2=WZZ2; YZZ2=WZZ2;
    X2D = zeros(Nv,Ntt); Y2D = X2D;
    for kv=1:Nv,
        ...
        if order==2,
            [x_eta2r,x_eta2i,coshkz,sinhkz,coshk,sinhk,expk,cost,sint] =...
                partkerng(x_w, x_k, x_e, theta, cosd, h, dBuv, z) ;
        end  
...
%        xyz1 = [zeros(1,3);xyz1;zeros(6*nfreq+7,3);conj(xyz1(nfreq:-1:1,:))] ;
        ...
        WZZ(kv,:) = flipud(xyz1(:,3));       
        XZZ(kv,:) = flipud(xyz1(:,1));
        YZZ(kv,:) = flipud(xyz1(:,2));
...
%            x_eta2 = [x_eta2(1:nl,:);zeros(4*nfreq+7,3);conj(x_eta2(nl:-1:2,:))] ;
            ...
            WZZ2(kv,:) = flipud(eta2(:,3));       
            XZZ2(kv,:) = flipud(eta2(:,1));
            YZZ2(kv,:) = flipud(eta2(:,2));
            X2D(kv,:)=deriv(1)*t';
            Y2D(kv,:)=deriv(2)*t';
        end
    end
...
X2.t=t';
Y2.t=t';
W.Z=permute(WZ,[2 1 3]); X.Z=permute(XZ,[2 1 3]); Y.Z=permute(YZ,[2 1 3]);
W2.Z=permute(WZ2,[2 1 3]); X2.Z=permute(XZ2,[2 1 3]); Y2.Z=permute(YZ2,[2 1 3]);
X2.drift=permute(X2drift,[2 1 3]); Y2.drift=permute(Y2drift,[2 1 3]);
