function [p, err, N] = cdfnormnd(x, mu, Sigma, errMax, ci, Nmax)
%CDFNORMND Multivariate normal cumulative distribution function (cdf).
%   P = CDFNORMND(X,MU,SIGMA) computes the multivariate normal cdf 
%   with mean vector MU and variance matrix SIGMA at the values in 
%   vector X.
%
%   P = CDFNORMND(X,MU,SIGMA,ERRMAX,CI,NMAX) uses additional control 
...
%
%   [P,ERR,N] = CDFNORMND(...) also returns the estimated error and the
...
while err > errMax && N < Nmax
    ...
    w = rand(m-1,1);
    for i = 2:m
        y(i-1) = invnorm(w(i-1)*f(i-1));
        q = 0;
        for j = 1:i-1
            q = q + C(i,j)*y(j);
        end;
        f(i) = cdfnorm((x(i) - q) / C(i,i)) * f(i-1);
    end;
    N = N + 1;
    del = (f(m) - p) / N;
    p = p + del;
