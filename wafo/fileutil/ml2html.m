function ml2html(rdir,htmldir,sdef,mknum,idxnum,intyp)
%ml2html  Produce html files for each m-file in root directory.
%
%  CALL: ml2html(rdir,htmldir,sdef,makernum,idxmaknum,typ);
%
...
% mknum      = the number for a makerfile (usually 1)
%              we assume that all makerfiles have names:
%              maker1.m, maker2.m, etc.
% idxnum     = the number for a IDX_makerfile (usually 1)
...
if nargin<4||isempty(mknum),   mknum   = 1;end
if nargin<5||isempty(idxnum),  idxnum  = 2;end

nt2 = 6; typ = [1 1 0 0 0 0];
if nargin>=6&&~isempty(intyp),   
  nt  = length(intyp); 
...
for ix =1:Nw
  ...
  refwrt(fid,dr,fullfile(htmlsdir{ix},'index.htm') ,1);
end
fprintf(fid,'%s \n','</UL>');
...
for ix=ind.'
  ...
  fprintf(fid,' <A HREF ="%s">%s</A> %s',fileref,mfiles(ix,:),b);
...
%  dr = strltok(W(ix).path,filesep);
  ...
  disp(['Writing an index file ' indexfile])
  ...
  for iz=find(drmask == ix) %1:length(W(ix).m),
    ...
    fprintf(fid,' <A HREF ="%s">%s</A> %s',htmlfiles{iz},mfiles(iz,:),b);
    if mod(iy,5)==0, fprintf(fid,'\n'),end
  end;
  fprintf(fid,'</pre>\n');
  fprintf(fid,'<HR></BODY>\n');
...
%  nr   = num2str(ix);
  for iy =1:length(W(ix).m)
    ...
    disp(['Check nr:  ' num2str(i) '   ' filename])
