...
% Updated 'Location' in 'legend' to work with Matlab 9.1, May 10, 2017 by GL 
% tested on Matlab 5.3, 8.1, 8.6, 9.1
% History:
...
switch lower(S.type(end-2:end))
  case {'enc','req','k1d'} % 1D plot
    ...
    if LegendOn
      if isfield(S,'CI'),
        legend(txt{:},txtCI,'Location','NorthEast')
      else
  ...
  case {'dir'}
    ...
    elseif (plotflag==2) || (plotflag==3),
      ...
      if LegendOn
        lh=legend(txtwdir,'Location','NorthEast');
      end
