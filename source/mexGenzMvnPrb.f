...
* and contains a mex-interface to Alan Genz's program,MVN distribution 
* subroutines SADMVN, KROMVN  subroutines
...
C
      ...
      integer V_pr, E_pr, IF_pr, C_pr, A_pr, B_pr
C-----------------------------------------------------------------------
C
      integer nlhs, nrhs
	...
	real*8 rIFT
      integer m, n, k, size
	DOUBLE PRECISION ABSEPS, RELEPS, VAL, ERR, INFINITY,rMETHOD
      INTEGER Nd, NN, MAXPTS, IFT, METHOD
...
C     Check to ensure the array is numeric (not strings).
     ...
     &   mxIsNumeric(prhs(6)) .eq. 0 .or.
     &   mxIsNumeric(prhs(7)) .eq. 0 ) then
...
*            if INFIN(I) = 2, Ith limits are [LOWER(I), UPPER(I)].		
	...
	rMETHOD = mxGetScalar(prhs(7)) 
	METHOD = rMETHOD 
C	call mxCopyPtrToInteger4(M_pr,MAXPTS,1)

C     Call the computational subroutine.
	IF  ( METHOD .LE. 1) THEN
           CALL SADMVN( N, LOW, UP, INFIN, COV, 
     &          MAXPTS, ABSEPS, RELEPS, ERR, VAL, IFT )
	ELSE IF (METHOD .EQ.2 ) THEN
           CALL KROMVN( N, LOW, UP, INFIN, COV, 
     &          MAXPTS, ABSEPS, RELEPS, ERR, VAL, IFT )
	ELSE IF (METHOD.EQ.3) THEN
           CALL RANMVN( N, LOW, UP, INFIN, COV, 
     &          MAXPTS, ABSEPS, RELEPS, ERR, VAL, IFT )
	ELSE IF (METHOD.GE.4) THEN
         CALL SPHMVN( N, LOW, UP, INFIN, COV, 
     &          MAXPTS, ABSEPS, RELEPS, ERR, VAL, IFT )
      ENDIF
