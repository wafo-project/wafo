...
C     minimum or level u separated maxima and minima + period/wavelenght       * 
C*******************************************************************************
...
!~~~~~~~~~~~~~~~~~
      ...
      IF (DEF.LE.1) THEN     ! Max2min and period/wavelength
         do i=2,Nx1
            xc(3,IJ+1:IJ+I-1) =  h(i)    
            xc(4,IJ+1:IJ+I-1) =  h(1:I-1) 
            IJ = IJ + I-1
            ! old call
            !do j=1,i-1 
            !   ij=ij+1
      ...
      ELSE
         ...
         do i=1,Nx1 
            xc(3,IJ+1:IJ+Nx1) =  h(i)              ! Max >= u
            xc(4,IJ+1:IJ+Nx1) =  h(Nx1+1:2*Nx1)    ! Min <= u
            IJ = IJ + Nx1
         enddo
         !do i=1,Nx1 
         !   do j=Nx1+1,2*Nx1 
...
!~~~~~~~~~~~~~~~~~~~~~~~~
            ij = 0
            do  i = 2, Nx1 
               ansr(i,1:i-1,1)=ansr(i,1:i-1,1)+fxind(ij+1:ij+i-1)*CC*dt
               ij=ij+i-1
               !do j = 1, i - 1 
               !   ij = ij + 1  
               !   ansr(i,j,1) = ansr(i,j,1)+fxind(ij)*CC*dt
               !end do 
            enddo 
         CASE (1) 
! joint density of (M,m,TMm)
            ij = 0
            do  i = 2, Nx1 
               ansr(i,1:i-1,Ntd)=fxind(ij+1:ij+i-1)*CC
               ij=ij+i-1
               !do j = 1, i - 1 
               !   ij = ij + 1  
               !   ansr(i,j,Ntd) = ansr(i,j,Ntd)+fxind(ij)*CC
               !end do 
            enddo
         CASE (2)
 ! joint density of level v separated (M,m)v
            IJ = 0
            do  i = 1,Nx1 
               ansr(i,:,1) = ansr(i,:,1)+fxind(ij+1:ij+Nx1)*CC*dt
               ij=ij+Nx1
               !do j = 1,Nx1 
               !   ij = ij+1  
               !   ansr(i,j,1) = ansr(i,j,1)+fxind(ij)*CC*dt      
 ...
 ! joint density of level v separated (M,m,TMm)v
            IJ = 0
            do  i = 1,Nx1 
               ansr(i,:,Ntd) = ansr(i,:,Ntd)+fxind(ij+1:ij+Nx1)*CC
               ij=ij+Nx1
               !do j = 1,Nx1 
               !   ij = ij+1  
               !   ansr(i,j,Ntd) = fxind(ij)*CC      
               !end do 
            enddo 
         END SELECT   
 ...
 200  do tn = Nstart,Ntime 
         ...
         do ts = 2,tn-1       
            CALL COV_INPUT2(BIG(1:Ntdc,1:Ntdc),tn,ts,R0,R1,R2,R3,R4) ! positive wave period
                                !print *,'Big='
...
! 4,  gives level u separated Max2min and wave period from Max to the crossing of level u (M,m,TMd).
                  ...
                  do  i = 1,Nx1 
                     do j = 1,Nx1 
                        ij = ij+1  
                        ansr(i,j,ts) = ansr(i,j,ts)+fxind(ij)*CC*dt
                     end do 
                  enddo 
               ENDIF
            CASE (5:)
...
! 5,  gives level u separated Max2min and wave period from the crossing of level u to the min (M,m,Tdm).
               ...
               do  i = 1,Nx1 
                  do j = 1,Nx1 
                    ij = ij+1  
                    ansr(i,j,tn-ts+1)=ansr(i,j,tn-ts+1)+fxind(ij)*CC*dt
                  end do 
               enddo 
               ENDIF
            END SELECT
...
! 4,  gives level u separated Max2min and wave period from Max to the crossing of level u (M,m,TMd).
               ...
               do  i = 1,Nx1 
                  do j = 1,Nx1 
                     ...
                     ansr(i,j,ts) = ansr(i,j,ts)+fxind(ij)*CC*dt
                     IF (ts.LT.tn-ts+1) THEN
                        ansr(j,i,tn-ts+1) =  ansr(j,i,tn-ts+1)+fxind(ij)*CC*dt ! exploiting the symmetry
                     ENDIF
                  end do 
               enddo 
            CASE (2)
! 5,   gives level u separated Max2min and wave period from the crossing of level u to min (M,m,Tdm).
               ...
               do  i = 1,Nx1 
                  do j = 1,Nx1 
                    ...
                    ansr(i,j,tn-ts+1)=ansr(i,j,tn-ts+1)+fxind(ij)*CC*dt
                     IF (ts.LT.tn-ts+1) THEN
                        ansr(j,i,ts) = ansr(j,i,ts)+fxind(ij)*CC*dt ! exploiting the symmetry
                     ENDIF
                  end do 
               enddo 
            END SELECT 
  ...
  !Cov(Xd,Xc)
       ...
       !Cov(Xd)  
        ...
        !Cov(Xd,Xc)
         ...
         IF (tnold.EQ.tn) THEN  ! A previous call to covinput has been made
