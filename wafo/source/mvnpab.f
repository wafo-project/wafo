...
************************************************************************
 ...
 10   VK(1) = ONE/P(NP)
      ...
      DO I = 1, SAMPLS
         ...
         VARSQR = ( I - 2.d0 )*VARSQR/DBLE(I) + DIFINT**2
      END DO
...
*
      ...
      DO K = 1, PRIME
         DO J = 1, NDIM
            ...
            X(J) = ABS( 2.d0*X(J) - 1.d0 )
         END DO
         SUMKRO = SUMKRO + ( FUNCTN(NDIM,X) - SUMKRO )/( 2.d0*K - 1.d0 )
         DO J = 1, NDIM
            X(J) = 1 - X(J)
         END DO
...
*
      ...
      INTEGER INFI(NL)
      DOUBLE PRECISION PROD, D1, E1, DI, EI, SUM1, PHINV, D, E, MVNNIT
      ...
      DO I = 1,N
         ...
         ELSE
            DI = ( 1 + SIGN( ONE, A(I+1)-SUM1 ) )/2.d0
            EI = ( 1 + SIGN( ONE, B(I+1)-SUM1 ) )/2.d0
         ENDIF
         PROD = PROD*(EI-DI)
      END DO
      MVNFNC = PROD
...
*
     ...
     &        ( (3.d0*LAM3-1.d0)*(5.d0*LAM3-3.d0)*35.d0 )
         W(LENRUL,1) = 1.d0/(3.d0*LAM3)**3/2.d0**NDIM
      ELSE
         LAM1 = ( LAM3*(15.d0 - 21.d0*LAM2) + 
     &        35.d0*(NDIM-1)*(LAM2-LAM3)/9.d0 )
     &       /  ( LAM3*(21.d0 - 35.d0*LAM2) + 
     &        35.d0*(NDIM-1.d0)*(LAM2/LAM3-1.d0)/9.d0 )
         W(6,1) = 1.d0/(4.d0*(3.d0*LAM3)**3.d0)
      ENDIF
      W(3,1) = ( 15.d0 - 21*(LAM3+LAM1) + 35*LAM3*LAM1 )
