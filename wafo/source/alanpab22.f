...
! fixed a bug in initfun funcmod00
! revised pab 23.11.2003
! revised pab 28.05.2003
! - added funcmod00, Nc1c2
...
!  Expectations
! The main RINDD is based on Podgorski et. al. (1999).
...
! Local variables
      ...
      DOUBLE PRECISION :: TWELVE, EXPMIN
      INTEGER :: I, LG, NG
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     ...
     &     abs(L).LE.MAX(-0.72d0*abs(H)+1.936d0,HALF)) THEN
         ...
         ! Alternating series: check if we can stop before SP=30
         ! (usually SP is less than 8)         
         do while ((abs(CN*L).GT.CON).AND.( SP < 30.d0))
            SN =  SP
            SP =  SP + ONE
            S2 =  S2 - W2
            W2 =  W2 * H2 / SP
...
! The absolute error is less than 1e-7.
      ...
      do while ((abs(CN*EX).GT.CON) .AND. (k < NCk)) 
...
!      DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE :: AA1,BB1
      ...
      ENDDO
      EPSL = MAX(EPS2,N*tmp*10.d0*TOL) !tmp
...
!     expected probability and interchange that variable with Kth.
         ...
         CVDIAG = ZERO
         RMAX   = ZERO
         IF ((Ndleft.GT.0) .OR. (NDIM < Nd+NIT)) THEN
            DO J = K, N1
               isXd = (INDEX1(J).GT.Nt)
               isOK = ((NDIM <= Nd+NIT).OR.isXd)
               IF ( R(J,J) .LE. K0*EPSL .OR. (.NOT. isOK)) THEN
...
!               YU = ZERO
!               IF (INFJ.GE.0) THEN
!                  IF (INFJ .NE. 0) YL =-EXP(-HALF*(AMIN*AMIN))*gSQTWPI1
!                  IF (INFJ .NE. 1) YU =-EXP(-HALF*(BMIN*BMIN))*gSQTWPI1
!               ENDIF
!               Y(K0) = ( YU - YL )/PRBMIN
!            ELSE
!               SELECT CASE (INFJ)
...
!      DEALLOCATE(BB1)
      ...
      SUBROUTINE COVSRT2(BCVSRT, Nt,Nd,R,Cm,A,B,INFI,INDEX1, 
     &     INFIS,INFISD, NDIM, Y, CDI )
      USE FIMOD
      USE SWAPMOD
      USE GLOBALCONST
...
!     expected probability and interchange that variable with Kth.
         ...
         IF ((Ndleft.GT.0) .OR. (NDIM < Nd+NIT)) THEN
            DO J = K, N1
               isXd = (INDEX1(J).GT.Nt)
               isOK = ((NDIM <= Nd+NIT).OR.isXd)
               IF ( R(J,J) .GT. EPSL .AND. isOK) THEN
                  TMP = ZERO    ! =  conditional mean of Y(I) given Y(1:I-1)
                  DO I = 1, K0 - 1
                     TMP = TMP + R(I,J)*Y(I)
                  END DO
...
! 75            CONTINUE
            ...
            IF (K1 .EQ.1) THEN
               ...
               IF (INFJ.GE.0) THEN
                  ...
                  IF  (INFJ.NE.1) FINB = 1
               ENDIF
               CALL C1C2(K1+1,N1,K0,A,B,INFI, Cm, R, 
     &              AMIN, BMIN, FINA,FINB)
...
!     IF (N < 10) EPSL = MIN(1D-10,EPS2)
      ...
      EPSL = MAX(EPS2,N*TMP*10.d0*LTOL)      
      IF (TMP.GT.EPSL) THEN
         DO I = 1, N
            IF ((INFI(I)  <  0).OR.(R(I,I).LE.LTOL)) THEN
               IF (INDEX1(I).LE.Nt)  THEN 
...
!BCVSRT.EQ.
                  ...
                  if (INFI(1)>=0) then
                     if (INFI(1).ne.0) then
                        AJ = -(R(I,JMIN))*MAX(A(I),-xCutOff)
                     endif
                     if (INFI(1).ne.1) then
                        BJ = - (R(I,JMIN))*MIN(B(I),xCutOff)
                     endif
                  endif
                  ...
                  AJ = (AJ + D*xCutOff)/CVDIAG
                  BJ = (BJ - D*xCutOff)/CVDIAG
...
! local variable
      ...
      MODULE FUNCVARMOD
!     Module transferring inputs tho mvnfun
!     BIG = ChOlesky Factor/Covariance matrix: 
!           Upper triangular part is the cholesky factor
!           Lower triangular part contains the conditional 
!           standarddeviations
!     CDI = Cholesky DIagonal elements
!     A,B = Integration limits
!     Cm  = conditional mean
!     INFIS, INFISD = Number of redundant variables of Xt and Xd respectively
      DOUBLE PRECISION, DIMENSION(:,:), ALLOCATABLE :: BIG 
      DOUBLE PRECISION, DIMENSION(:  ), ALLOCATABLE :: CDI 
      ...
      INTEGER,SAVE :: INFIS,INFISD 
      END MODULE FUNCVARMOD
      MODULE FUNCMOD00            
      USE FUNCVARMOD
      IMPLICIT NONE     
! FUNCTION module containing constants transfeered to mvnfun
...
! variables set in initfun and used in mvnfun:
      ...
      USE GLOBALCONST
      USE FIMOD
      USE COVSORTMOD, ONLY: C1C2, printvecd, printveci
...
! local variables:
      ...
      DOUBLE PRECISION :: AI, BI, R12, R13, R23
      DOUBLE PRECISION :: xCut 
...
!            if INFIN(I) = 2, Ith limits are [A(I), B(I)].
         ...
         DO J=1, INFIS + INFISD
            I = I + 1
            IF (INFI(I).GE.0) THEN
               IF ((INFI(I).NE.0).AND.(EPS  < A(I))) GOTO 200
...
!     Print *,'rindd ndim1',Ndim1
      ...
      ENDIF
!     
      IF (xCut.NE.gZERO) THEN
         J = 1
         DO I = 2, N+1
            IF (BIG(J+1,I).GT.gZERO) THEN
               J = J + 1
            ELSE
               ! Add xCut std to deterministic variables to get an upper
               ! bound for integral
              A(I) =  A(I) - xCut * BIG(I,J)
              B(I) =  B(I) + xCut * BIG(I,J)
            ENDIF
         END DO
      ENDIF
 ...
 100     IF (INDEX1(I).GT.Nt) THEN ! Save the mean for Xd
         ...
         IF (I.EQ.N+1.OR.BIG(2,I+1).GT.gZERO) THEN
            IF (useC1C2.AND.I<N) THEN
               Y(:) = gZERO
               CALL C1C2(I+1,N+1,1,A,B,INFI,Y,BIG,AI,BI,FINA,FINB)
!               CALL C1C2(I+1,N+1,A,B,INFI, Y, BIG(1,:),BIG(:,1),
!     &              AI, BI, FINA,FINB) 
            ENDIF
            CALL MVNLMS( AI, BI,2*FINA+FINB-1, D1, E1 )
            IF (D1.GE.E1) GOTO 200
            IF ( NdleftO.LE.0) THEN
               IF (Nd.GT.0) VAL0 = JACOB(xd,xc) 
               SELECT CASE (I-N)
               ...
               CASE (0)     !ELSEIF (I.EQ.N) THEN
                  ...
                  D1 = SQRT( gONE + BIG(1,I+1)*BIG(1,I+1) ) 
                  INFI(2) = INFI(I+1)
                  A(1) = AI
                  B(1) = BI
                  INFI(1) = 2*FINA+FINB-1
                  IF ( INFI(2) .NE. 0 ) A(2) = A(I+1)/D1
                  IF ( INFI(2) .NE. 1 ) B(2) = B(I+1)/D1
...
! TODO :this does not work correctly yet! 
                  ...
                  D1 = gONE/SQRT( gONE + BIG(1,I+1)*BIG(1,I+1) )
                  R12 = BIG( 1, I+1 ) * D1
                  if (BIG(3,I+2)>gZERO) then
                     E1 = gONE/SQRT( gONE + BIG(1,I+2)*BIG(1,I+2) +
     &                    BIG(2,I+2)*BIG(2,I+2) )                
                     R13 = BIG( 1, I+2 ) * E1
                     ...
                     R13 = BIG( 1, I+2 ) * E1
                     R23 = E1*D1 + R12 * R13
                     IF ((E1  <  gZERO).AND. INFI(I+2).GE. 0) THEN
                        CALL SWAP(A(I+2),B(I+2))
                        IF (INFI(I+2).NE. 2) INFI(I+2) = 1-INFI(I+2)
                     END IF
                     !R23 = BIG( 2, I+2 ) * (E1 * D1) + R12 * R13
                  endif
                  INFI(2) = INFI(I+1)
                  ...
                  IF ( INFI(2) .NE. 0 ) A(2) = A(I+1) * D1
                  IF ( INFI(2) .NE. 1 ) B(2) = B(I+1) * D1
                  IF ( INFI(3) .NE. 0 ) A(3) = A(I+2) * E1
                  IF ( INFI(3) .NE. 1 ) B(3) = B(I+2) * E1
                  if(.false.) then
                     CALL PRINTVECD((/R12, R13, R23 /),'R12 = ')
                     CALL PRINTVECD((/D1, E1 /),'D1 = ')
                     CALL PRINTVECD(BIG(1,1:3),'BIG(1,1:3) = ')
                     CALL PRINTVECD(BIG(2,2:3),'BIG(2,2:3) = ')
                     CALL PRINTVECD(BIG(1:3,1),'BIG(1:3,1) = ')
                     CALL PRINTVECD(BIG(2:3,2),'BIG(2:3,2) = ')
                     CALL PRINTVECD(A(1:I+2),'A = ')
                     CALL PRINTVECD(B(1:I+2),'B = ')
                     CALL PRINTVECI(INFI(1:I+2),'INFI = ')
                     CALL PRINTVECI(INDEX1(1:I+2),'index1 = ')
                  endif
                  VALUE = TVNMVN( A, B,INFI,
     &                 (/R12, R13, R23 /),ABSEPS) * VAL0
                  GOTO 200
                  ENDIF
               END SELECT !ENDIF
            ENDIF
...
!
      ...
      USE GLOBALDATA, ONLY: EPS2,EPS,xCutOff,Nc1c2
      IMPLICIT NONE
      INTEGER, INTENT (IN) :: Ndim
...
!      xCut = gZERO ! xCutOff
      ...
      useC1C2 = (IK+1.LE.Nc1c2)
      IF (useC1C2) THEN
         ! Calculate the conditional mean
         ! E(Y(I) | Y(1),...Y(I0))/STD(Y(I)|Y(1),,,,Y(I0))
         Y(I0+1:N+1) = BIG(IK, I0+1:N+1)*Y(IK) 
      ENDIF
     ...
     &        Y(IK) * CDIXd(NdleftN+1:NdleftO)
      ...
      DO I = I0+1, N+1
         ...
         END IF
 100     isXd = (Nt<INDEX1(I))
         IF (isXd) THEN 
!     Save the mean of xd and Covariance diagonal element 
            ! Conditional mean E(Xi|X1,..X)
            CmXd(NdleftN)  = Cm(I) + TMP * CDI(I) 
             ! Covariance diagonal 
            CDIXd(NdleftN) = CDI(I)              
            NdleftN        = NdleftN - 1
         END IF 
         IF (I == N+1 .OR. BIG(IK+1,I+1) .GT.0.D0 ) THEN 
            IF (useC1C2) THEN
!     Must change so that Y = conditional expectation, in order to use
!     C1C2  here:
               CALL C1C2(I+1,N+1,IK,A,B,INFI,Y,BIG,AI,BI,FINA,FINB)
            ENDIF
     ...
     &                 Y(IK) * CDIXd(NdleftN+1:NdleftO)
               ...
               IF (useC1C2) THEN
                  ...
                  Y(I+1:N+1) = Y(I+1:N+1) + BIG(IK, I+1:N+1)*Y(IK)
               ENDIF
            ENDIF            
            IK   = IK + 1
            FINA = 0
...
! variables set in initfun and used in mvnfun:
      ...
      USE FIMOD
      USE COVSORTMOD, ONLY: C1C2
      USE GLOBALDATA, ONLY: NIT,EPS2,EPS,xCutOff,ABSEPS
      IMPLICIT NONE
!      INTEGER, INTENT(IN) :: NDIM
      DOUBLE PRECISION, INTENT(OUT) :: VALUE
      INTEGER, INTENT(out) :: INFORM
...
!     &           AI, BI, FINA,FINB) 
            ...
            IF ( NdleftO.LE.0) THEN
               ...
               CASE ( -1 )  !ELSEIF (I.EQ.N-1) THEN
                  ...
                  !1/E1=  STD(X(I+2)|X(1)X(I+1))
                  D1 = SQRT( gONE + BIG(1,I+1)*BIG(1,I+1) )
                  E1 = SQRT( gONE + BIG(1,I+2)*BIG(1,I+2) +
     &                 BIG(2,I+2)*BIG(2,I+2) )
                  INFI(2) = INFI(I+1)
                  INFI(3) = INFI(I+2)
                  A(1) = AI
...
!     &           AI, BI, FINA,FINB) 
            ...
            IF ( I .LE. N .OR. NdleftN < NdleftO) THEN
               Y(IK) = FIINV( DI + W(IK)*( EI - DI ) )
               IF (NdleftN < NdleftO ) THEN
                  xd(NdleftN+1:NdleftO) = CmXd(NdleftN+1:NdleftO)+
     &                 Y(IK)*CDIXd(NdleftN+1:NdleftO)
...
! variables set in initfun and used in mvnfun:
      ...
      USE GLOBALCONST
      USE FIMOD
      USE GLOBALDATA, ONLY: EPS2,EPS,xCutOff,ABSEPS
      IMPLICIT NONE
!      INTEGER, INTENT(IN) :: NDIM
      DOUBLE PRECISION, INTENT(OUT) :: VALUE
      INTEGER, INTENT(out) :: INFORM
         ...
         IF (I.EQ.N+1.OR.BIG(2,I+1).GT.ZERO) THEN
            ...
            ELSEIF (N.EQ.1.AND.NdleftO.LE.0) THEN
               IF ( ABS( BIG(2,I+1) ) .GT. ZERO ) THEN
                  AP1 = SQRT( ONE + BIG(1,I+1)*BIG(1,I+1))
					INFI(2) =INFI(I+1)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &	              indI,Blo,Bup,INFIN,XcScale)  
      ...
      USE SWAPMOD
      USE COVSORTMOD
      USE FUNCMOD00        ! use FIINV AND C1C2 (REGRESSION EQ) Nc1c2 times
!      USE FUNCMOD0       ! use FIINV AND C1C2 (REGRESSION EQ) 1 time
         ...
         ELSE
            ...
            DO J = 1, Nc        !Iterative conditioning on the last Nc variables  
               ...
               IF ( XcPrb .GT. gZERO) THEN
                  XcMin = -EXP(-gHALF*(XcMin*XcMin))*gSQTWPI1
                  XcMax = -EXP(-gHALF*(XcMax*XcMax))*gSQTWPI1
            ...
            ENDDO            
         END IF
	! compute average integrationlimits 	
...
!     Iterative conditioning on the last Nc variables  
            ...
            ERROR = ERROR*fxc
            !if fxc small:  don't bother calculating it, goto end
            IF (fxc  < fxcEpss) GOTO 100 
         END IF
...
!     NOTE: A and B are integration limits with Cm subtracted
         ...
         IF (BCVSRT) THEN       
!     conditionally sorted before hand just scale integration limits
            ...
            DO I = 1,Ntd
               IF ( CDI(I) < gZERO.AND.INFI(I).GE.0) THEN
                  CALL SWAP( A(I), B(I) ) 
...
!     print *, 'Ndim too large for SADMVN => Calling KRBVRC'
     ...
     &           RELEPS, ERROR, VALUE, INFORM )
	   CASE (6)       !       Call the scrambled Sobol sequence rule integration procedure
	      CALL SOBNIED( NDIM, MINPTS, MAXPTS, MVNFUN, LABSEPS, RELEPS,
     &           ERROR, VALUE, INFORM )
	   CASE (7:)
	      CALL DKBVRC( NDIM, MINPTS, MAXPTS, MVNFUN, LABSEPS, RELEPS,
     &           ERROR, VALUE, INFORM )
         END SELECT   
