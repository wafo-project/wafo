...
%
% 1997-10-22  PJ  Korrigerat pcolor-plot. Ritar nu r�tt antal rutor.
...
% Check input arguments
  ...
  % Vrid cykelmatrisen f�r att plotta r�tt
  %F = flipud(F');
...
function _cmatplot(ux,uy,F,method,clevels)
  ...
  if method == 1      % mesh
    F = flipud(F');% Vrid cykelmatrisen f�r att plotta r�tt
    mesh(ux,fliplr(uy),F);
    xlabel('min');
    ylabel('Max');
    view(-37.5-90,30);
  ...
  elseif method == 4  % contour
    ...
    axis('square');
    % Skriv h�jden p� konturlinjerna. (Kopia fr�n WAT.)
  ...
  %  title(['ISO-lines: ' Cstr])
    ...
    if 1==1 % Skriv ut h�jden p� niv�kurverna.
      clevels=sort(clevels);
      n_clevels=length(clevels);
      if n_clevels>12
        disp('   Only the first 12 levels will be listed in table.');
