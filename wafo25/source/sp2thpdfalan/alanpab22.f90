...
REAL(KIND=STND) :: EPS2=1.d-10   !- EPS2, the variable is 
											 !  considered deterministic 
REAL(KIND=STND) :: EPS  = 1.d-5    ! = SQRT(EPS2)
REAL(KIND=STND) :: EPS0 = 1.d-10  
REAL(KIND=STND) :: XCEPS2=1.d-16 ! if Var(Xc) is less return NaN
REAL(KIND=STND) :: AbsEps = 1.d-3 ! requested absolute error 
REAL(KIND=STND) :: RelEps = 1.d-10 ! requested Relative error, i.e. if 
...
REAL(KIND=STND) :: fxcEpss=1.d-20 ! if less do not compute E(...|Xc)
REAL(KIND=STND) :: xCutOff=5.d0  ! upper/lower truncation limit of the 
											! normal CDF 
REAL(KIND=STND) :: FxCutOff  = 0.99999942669686d0 
REAL(KIND=STND) :: CFxCutOff = 5.733031438470704d-7       ! 1-FxCutOff, 
REAL(KIND=STND) :: XSMALL=4.2D-16, XMAX=8.29287554336168D0 !cut off parameters to FI
...
MODULE GLOBALCONST        ! global constants
	IMPLICIT NONE
	PUBLIC
   REAL(KIND=STND), PARAMETER :: gSQTWPI1= 0.39894228040143d0 !=1/sqrt(2*pi)
   REAL(KIND=STND), PARAMETER :: gSQPI1  = 0.56418958354776d0 !=1/sqrt(pi)
   REAL(KIND=STND), PARAMETER :: gSQPI   = 1.77245385090552d0 !=sqrt(pi)
   REAL(KIND=STND), PARAMETER :: gSQTW   = 1.41421356237310d0 !=sqrt(2)
   REAL(KIND=STND), PARAMETER :: gSQTW1  = 0.70710678118655d0 !=1/sqrt(2)
   REAL(KIND=STND), PARAMETER :: gPI1    = 0.31830988618379d0 !=1/pi
   REAL(KIND=STND), PARAMETER :: gPI     = 3.14159265358979D0 !=pi
   REAL(KIND=STND), PARAMETER :: gTWPI   = 6.28318530717958D0 !=2*pi
   REAL(KIND=STND), PARAMETER :: gSQTWPI = 2.50662827463100D0 !=sqrt(2*pi)
   REAL(KIND=STND), PARAMETER :: gONE    = 1.d0
	REAL(KIND=STND)   :: gINFINITY =  SQRT( -2.0D0 * LOG(1.e+12 * TINY(gONE)) )
	! Set xinfin (infinity).
...
*
      ...
      REAL(KIND=STND) :: VAL
!local variables
      REAL(KIND=STND) SPLIT1, SPLIT2, CONST1, CONST2, 
...
*     
     ...
     *                      *R + F2 )*R + F1 )*R + 1.d0 )
      ...
      use GLOBALDATA, only: XMAX,CFxCutOff
      REAL(KIND=STND), INTENT(IN) :: P1
      REAL(KIND=STND) :: VALUE
! Local variables
      REAL(KIND=STND), DIMENSION(4) :: A,B,C
      REAL(KIND=STND), DIMENSION(2) :: D
      REAL(KIND=STND) :: P,Z,X=0.d0
      REAL(KIND=STND), PARAMETER :: P0=0.7D0
       REAL(KIND=STND), PARAMETER :: SQTWO=1.41421356237310d0    !=sqrt(2)
...
!x = x - (2.d0*FI(x*sqrt(2.d0))-1.d0 - p) / (2/sqrt(pi) * exp(-x^2));
      ...
      IMPLICIT NONE
      REAL(KIND=STND) :: X,XX,value,z,y,del
      REAL(KIND=STND) :: xbreak=0.46875d0
!  MACHINE-DEPENDENT PARAMETERS  put into the GLOBALDATA module                                        
!------------------------------------------------------------------     
!     REAL(KIND=STND), PARAMETER :: XSMALL=4.2D-16, XMAX=9.269D0        
...
! Latest revision of Fortran 90 version - 12 August 1997
	...
	REAL(KIND=STND), INTENT(OUT), OPTIONAL :: P, Q, PDF
!Local variables
	REAL(KIND=STND) :: PP, QQ, PPDF
	REAL(KIND=STND) :: P0, P1, P2, P3, P4, P5, P6, 
     *     Q0, Q1, Q2, Q3, Q4, Q5, Q6, Q7,
     *     EXPNTL, CUTOFF, ROOTPI, ZABS, ONE, ZERO, HALF
      PARAMETER(
...
!     |Z| <= 37
!     
      EXPNTL = EXP( -HALF*ZABS**2)
	PPDF   = EXPNTL/ROOTPI
	IF (PRESENT(PDF)) PDF = PPDF

      IF ( ZABS .LT. CUTOFF ) THEN
		!     |Z| < CUTOFF = 10/SQRT(2)    
         PP = EXPNTL*( (((((P6*ZABS + P5)*ZABS + P4)*ZABS + P3)*ZABS
     &      + P2)*ZABS + P1)*ZABS + P0)/(((((((Q7*ZABS + Q6)*ZABS
     &      + Q5)*ZABS + Q4)*ZABS + Q3)*ZABS + Q2)*ZABS + Q1)*ZABS
     &      + Q0 ) 
      ELSE   
	! |Z| >= CUTOFF.
         PP = PPDF/( ZABS + ONE/( ZABS + 2.d0/( ZABS + 3.d0/( ZABS 
     &                   + 4.d0/( ZABS + 0.65D0 ) ) ) ) )
      END IF
	IF (Z .LT. ZERO) THEN
		IF (PRESENT(P)) P = PP
...
*     
     ...
     *                        + 4.d0/( ZABS + 0.65D0 ) ) ) ) )/ROOTPI
      ...
      REAL(KIND=STND), INTENT(out) :: LOWER, UPPER
...
*
      ...
      REAL(KIND=STND)  :: VAL
! Local variables
      REAL(KIND=STND) :: ZERO,HALF,ONE,TWO,TWOPI 
      INTEGER :: I, LG, NG
      PARAMETER ( ZERO = 0.D0,ONE=1D0,TWO=2D0,HALF=0.5D0)
      PARAMETER (TWOPI = 6.2831 85307 179586D0 ) 
      REAL(KIND=STND), DIMENSION(10,3) :: X, W
      REAL(KIND=STND) :: AS, A, B, C, D, RS, XS
      REAL(KIND=STND) :: SN, ASR, H, K, BS, HS, HK
...
*     Gauss Legendre Points and Weights, N = 20
     ...
     *  0.1527533871307259D+00,-0.7652652113349733D-01/
     ...
     +                       - ( ONE + C*XS*( ONE + D*XS ) ) )
      ...
      IMPLICIT NONE
      REAL(KIND=STND), INTENT(in) :: H1, L1, GH
      REAL(KIND=STND)             :: T                                     
!                                                                       
!     Local constants                                                   
!     
      REAL(KIND=STND), PARAMETER :: SQPI = 1.77245385090552d0   !=sqrt(pi)
      REAL(KIND=STND), PARAMETER :: SQTWO= 1.41421356237310d0    !=sqrt(2)
      REAL(KIND=STND), PARAMETER :: PI1  = 0.31830988618379d0     !=1/pi
      REAL(KIND=STND), PARAMETER :: PI   = 3.14159265358979D0     !=pi
      REAL(KIND=STND), PARAMETER :: ZERO = 0.D0
      REAL(KIND=STND), PARAMETER :: QUART= 0.25D0
      REAL(KIND=STND), PARAMETER :: HALF = 0.5D0
      REAL(KIND=STND), PARAMETER :: ONE  = 1.D0
      REAL(KIND=STND), PARAMETER :: TWO  = 2.D0
      REAL(KIND=STND), PARAMETER :: EXPLIM=80.D0
      INTEGER,          PARAMETER :: NCk=7
      REAL(KIND=STND), DIMENSION(NCk) :: Ck
...
! Local variables
      INTEGER :: k
      REAL(KIND=STND) :: EPSL,H,L, H2, CON, CN, 
...
!      print *,'THL K=',K                      
      ...
      IMPLICIT NONE
      REAL(KIND=STND), INTENT(in) :: a1,b1,a2,b2,R
      REAL(KIND=STND)             :: prb
! Local variables
      REAL(KIND=STND), PARAMETER :: PI1  = 0.31830988618379d0 !=1/pi
      REAL(KIND=STND) :: L1,L2,L3,L4,Ga1,Ga2,Gb1,Gb2,SQR,
...
!         IF (abs(R).GT.1.d0+EPSL) PRINT *,'Warning: correlation, R=',R
 ...
 200  if (prb > 1.d0) then ! fix up round off error
      ...
      IMPLICIT NONE
      REAL(KIND=STND), INTENT(in) :: X1,X2,A,B,C,D
      REAL(KIND=STND)             :: value
!Local variables
      REAL(KIND=STND)             :: Y1,Y2,Y3
       REAL(KIND=STND), PARAMETER :: SQTWOPI1=3.9894228040143d-1 !=1/sqrt(2*pi)
      ...
      IMPLICIT NONE
      REAL(KIND=STND), INTENT(in) :: X1,X2,A,B
      REAL(KIND=STND)             :: value
! Local variables
      REAL(KIND=STND)             :: X0,Y0,Y1,Y2
      REAL(KIND=STND), PARAMETER :: SQTWOPI1=3.9894228040143d-1 !=1/sqrt(2*pi)
      ! Let  X  be standardized Gaussian variable, 
      ! i.e., X=N(0,1). The function calculate the
      ...
      IMPLICIT NONE
      REAL(KIND=STND), INTENT(in) :: A, X1, X2
      REAL(KIND=STND), INTENT(out) :: LOWER, UPPER,Ca,Pa
      INTEGER,INTENT(in) :: INFIN
      ! Local variables
      REAL(KIND=STND) :: P1
      REAL(KIND=STND), PARAMETER :: AMAX = 5D15
...
!     Pa   = excdf(-A,A) probability at the inflection point of the CDF
      REAL(KIND=STND), intent(in) :: P,A,Ca,Pa
      REAL(KIND=STND) :: val
! local variables
      REAL(KIND=STND), parameter :: amax = 5.D15
      REAL(KIND=STND), parameter :: epsl = 5.D-15
      REAL(KIND=STND), parameter :: xmax = 8
      REAL(KIND=STND) :: P1,Xk,Ak,Zk,SGN
...
! The inverse cdf of 0 is -inf, and the inverse cdf of 1 is inf.  
     ...
     &              (Zk+P1)))
      ...
      REAL(KIND=STND), intent(in) :: xk0,ak,CaP ! =Ca*P
      REAL(KIND=STND) :: xk
!Local variables                                
      INTEGER,          PARAMETER :: ixmax = 25
      REAL(KIND=STND), parameter :: crit = 7.1D-08 ! = sqrt(1e-15)
      REAL(KIND=STND), parameter :: SQTWOPI1 = 0.39894228040143D0 !=1/SQRT(2*pi)
      REAL(KIND=STND), parameter :: SQTWOPI = 2.50662827463100D0 !=SQRT(2*pi)
      INTEGER :: IX
      REAL(KIND=STND) :: H,H1,tmp0,tmp1,XNEW
...
!      END INTERFACE
      ...
      REAL(KIND=STND), DIMENSION(:  ), ALLOCATABLE :: COF,CDI  ! ChOlesky Factor and Cholesky DIagonal elements
      REAL(KIND=STND), DIMENSION(:  ), ALLOCATABLE :: Cm,CDIXd,CmXd
      REAL(KIND=STND), DIMENSION(:  ), ALLOCATABLE :: xd,xc,Y
      REAL(KIND=STND), DIMENSION(:  ), ALLOCATABLE :: A,B      ! Integration limits
      INTEGER, DIMENSION(:  ), ALLOCATABLE :: INFI,INDEX1      
      INTEGER :: Nt,Nd,INFIS,INFISD    ! Size information
      INTEGER :: I0,NdleftN0,IJ0
      REAL(KIND=STND) :: E1,D1
! variables transfeered to mvnfun1:
      REAL(KIND=STND) :: VAL0,AA0,Ca0,Pa0
      ...
      IMPLICIT NONE
      REAL(KIND=STND), DIMENSION(:),INTENT(in) :: xd0 ,xc0
      REAL(KIND=STND) :: value1
...
!      INTEGER, INTENT(IN) :: NDIM
      REAL(KIND=STND), INTENT(OUT) :: VALUE
      INTEGER, INTENT(out) :: INFORM
! local variables:
      INTEGER ::  N,NdleftO
      INTEGER :: I, J,  INFA, INFB
      REAL(KIND=STND) :: AI, BI
*     
...
! INDEX1 - if INDEX1(I)>Nt then variable no. I is one of the Xd variables otherwise it is one of Xt
 ...
 200  INFORM = 1
	...
	USE JACOBMOD
      ...
      IMPLICIT NONE
      INTEGER, INTENT (in) :: Ndim
      REAL(KIND=STND), DIMENSION(:), INTENT(in) :: W
      REAL(KIND=STND) :: VAL
! local variables:
      INTEGER ::  N
      INTEGER :: I, J, IJ,  INFA, INFB,Ndleft,LK,JMX,IK
      REAL(KIND=STND) :: TMP, AI, BI, DI, EI,TMPOLD
      REAL(KIND=STND), PARAMETER :: EPSL=1D-12
*     
...
! local variables:
      ...
      REAL(KIND=STND) :: TMP, AI, BI, DI, EI
      REAL(KIND=STND), PARAMETER :: EPSL=1D-12
*     
*     Integrand subroutine
*
...
!*****************************************************
	...
	INTERFACE COVSRT0
      ...
      USE FIMOD
      IMPLICIT NONE
      REAL(KIND=STND) , INTENT(in) :: dREPS,dAEPS,dEPS2,dXc 
	...
	PRINT *, 'NsimMax = ', NsimMax
      ...
      INTEGER ::j
      REAL(KIND=STND),DIMENSION(:,:)::array
      DO j=1,size(array,1)
         PRINT 111,j,array(j,:)
111      FORMAT (i2,':',10F10.5)
      ...
      REAL(KIND=STND), DIMENSION(:),INTENT(in) :: A,B,COF
...
!	USE FUNCMOD1  ! use EXINV
      ...
      USE KROBOVMOD
      IMPLICIT NONE  
      REAL(KIND=STND), DIMENSION(:  ), INTENT(out):: fxind 
      REAL(KIND=STND), DIMENSION(:,:), INTENT(in) :: BIG1
      REAL(KIND=STND), DIMENSION(:,:), INTENT(in) :: xc1 
      REAL(KIND=STND), DIMENSION(:  ), INTENT(in) :: Ex            
      REAL(KIND=STND), DIMENSION(:,:), INTENT(in) :: Blo, Bup  
      INTEGER,          DIMENSION(:  ), INTENT(in) :: indI,INFIN
      INTEGER, INTENT(in) :: Nt1 
! local variables
      REAL(KIND=STND), DIMENSION(:,:), ALLOCATABLE :: BIG
     ...
     &     Nc,Ntdc,Ntd,Nx,NDIM,NDIM1,Ndleft,MAXPTS,MINPTS, NLHD,I
      REAL(KIND=STND)         :: VALUE,SQ0,xx,fxc,quant,ERROR
      LOGICAL :: BCVSRT=.TRUE.    
...
!               print * ,'Rindscis: only able to useMIDP if rateLHD>0'
 ...
 110  CONTINUE
      ...
      REAL(KIND=STND), DIMENSION(:), INTENT(in)  :: Hlo,Hup
      INTEGER,          DIMENSION(:), INTENT(in)  :: INFIN
      REAL(KIND=STND),               INTENT(in)  :: SQEPS
      INTEGER,                        INTENT(out) :: INFORM
! LOCAL variable
      INTEGER :: J,K,N
...
!            if INFIN(I) = 2, Ith limits are [Hlo(I), Hup(I)].
      ...
      USE GLOBALDATA, ONLY : xCutOff
      IMPLICIT NONE
      REAL(KIND=STND), DIMENSION(:  ), INTENT(in)  :: xc,Cm 
      INTEGER,          DIMENSION(:  ), INTENT(in)  :: xedni,indI,INFIN    
      REAL(KIND=STND), DIMENSION(:,:), INTENT(in)  :: Blo,Bup   
      REAL(KIND=STND), DIMENSION(:  ), INTENT(out) :: Hlo,Hup
...
!      print * ,'barrier hlo:',size(Hlo),Hlo(xedni(1:indI(NI)))
      ...
      REAL(KIND=STND), DIMENSION(:,:), INTENT(inout) :: R
      INTEGER,          DIMENSION(:  ), INTENT(inout) :: index1 
      INTEGER, INTENT(out) :: Nst,Nsd,INFORM 
! local variables
      REAL(KIND=STND), DIMENSION(:  ), allocatable   :: SQ
...
!      END DO
 ...
 200  DEALLOCATE(SQ)
      ...
      IMPLICIT NONE
      REAL(KIND=STND), DIMENSION(:,:), INTENT(inout) :: R
      REAL(KIND=STND), DIMENSION(:),   INTENT(inout) :: SQ 
      INTEGER,          DIMENSION(:  ), INTENT(inout)   :: index1 
...
! local variables
      REAL(KIND=STND), DIMENSION(Ntdc)  :: SQ
      INTEGER :: r1,c1,r2,c2
      LOGICAL :: changed
! R         = Input: Cov(X) where X=[Xt Xd Xc] is stochastic vector
!            Output: sorted Conditional Covar. matrix   Shape N X N  (N=Ntdc=Nt+Nd+Nc) 
...
*
      INTEGER,                          INTENT(in)    :: Nt,Nd
      REAL(KIND=STND), DIMENSION(:,:), INTENT(in)    :: R
      REAL(KIND=STND), DIMENSION(:  ), INTENT(inout) :: Cm,A,B
      INTEGER,          DIMENSION(:  ), INTENT(inout) :: INFI
      INTEGER,          DIMENSION(:  ), INTENT(inout) :: INDEX1
      INTEGER,                          INTENT(out) :: INFIS,INFISD,NDIM
      REAL(KIND=STND), DIMENSION(:  ), INTENT(out)   :: Y, COF, CDI
! Local variables
      INTEGER :: N,I, J, K, L, M, II, IJ, IL, JMIN,Ndleft
      REAL(KIND=STND) :: SUMSQ, AJ, BJ, TMP, SQTWPI, D, E,EPSL 
      REAL(KIND=STND) :: CVDIAG, AMIN, BMIN, DMIN, EMIN, YL, YU
...
! Local variables
	...
	INTEGER :: KK, K1, KK1, K0, Nullity
      REAL(KIND=STND) :: SUMSQ, AJ, BJ, TMP, SQTWPI, D, E,EPSL, TOL
	REAL(KIND=STND) :: AA, Ca, Pa
      REAL(KIND=STND) :: CVDIAG, AMIN, BMIN, DMIN, EMIN, YL, YU
...
*
      REAL(KIND=STND), DIMENSION(:),INTENT(inout):: A, B, C
...
*
      REAL(KIND=STND), DIMENSION(:),INTENT(inout):: A, B, C, Cm
...
! local variables
	...
	CALL SWAPINT(IND(P),IND(Q))
      ...
      IMPLICIT NONE
      REAL(KIND=STND), INTENT(inout) :: m,n
      REAL(KIND=STND)                :: tmp      
      tmp=m
      m=n
      n=tmp
