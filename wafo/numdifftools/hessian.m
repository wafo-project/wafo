function [hess,err] = hessian(fun,x0,varargin)
%HESSIAN Estimate Hessian matrix i.e., array of 2nd order partial derivatives
% 
% CALL [hess,err] = hessian(fun,x0,options)
% 
%  hess    = Hessian matrix of fun, evaluated at x0. size N x N
%  err     = error estimates corresponding to each element in hess. size N x N
...
%        Must be a integer from the set [1,2,3,4].  DEFAULT: 3 (Three Romberg
%        terms)
%  .StepRatio       : Ratio used between sequential steps in the estimation
%                    of the derivative (DEFAULT: 2)
%
% HESSIAN estimate the array of 2nd order partial derivatives for FUN
% evaluated at X0.  HESSIAN is NOT a tool for frequent use on an expensive
% to evaluate objective function, especially in a large number of dimensions.
% Its computation will use roughly  O(6*n^2) function evaluations for n 
% parameters.
%
% Examples:
%  %Rosenbrock function, minimized at [1,1]
%  rosen = @(x) (1-x(1)).^2 + 105*(x(2)-x(1).^2).^2;
%  [h,err] = hessian(rosen,[1 1]) %  h =[ 842 -420; -420, 210];
%
%  %cos(x-y), at (0,0)
%  opts = hessian('defaults');
%  opts.RombergTerms = 4;
%  h2 = hessian(@(xy) cos(xy(1)-xy(2)),[0 0]) % h2 = [-1 1; 1 -1];
%  h2 = hessian(@(xy) cos(xy(1)-xy(2)),[0 0],opts)
%
...
params = parseoptions(params,varargin{:});
...
rombexpon = 2*(2:params.RombergTerms)  - 2;
...
for i = 2:nx
  for j = 1:(i-1)
    ...
    [hess(i,j),err(i,j)] =  rombextrap(params.StepRatio,dij,rombexpon); %[2 4]);
    hess(j,i) = hess(i,j);
    err(j,i) = err(i,j);
  end
end
