%README Readme file for module GRAPHUTIL in WAFO Toolbox 
% Version 2.5
%
%  All XXXfigs.m function files are renamed to figXXX.m
% 
% Version 2.2.1   07-Nov-2007
%
%  All plot related functions that used to be in the MISC module
%  is moved to this module:
%   choices      - Create a list of choices with uicontrols and callbacks.
%   choicex      - Close a list of choices.
%   cyclefigs    - Cycle through figure windows.
%   restorefigs  - Restore figures window size and position to its default value.
%   sortfigs     - Sort figure windows in ascending order.
%   stackfigs    - Stack figure windows
%   tilefigs     - Tile figure windows.
%   maximizefigs - Maximize figure(s) window size
%   cltext       - Places contour level text in the current window
%   figtext      - Places text in figure window.
%   wafostamp    - Prints a caption "WAFO" in current figure.
%   clevels      - Extract the contour levels from the contour matrix
%   fcolorbar    - Display colorbar with discrete color axis for filled contour plot
%   fwaitbar     - Fast display of wait bar.
%
%  NEW FUNCTIONS
%  ~~~~~~~~~~~~~
%   mayaplot
%   pilefigs     - Pile figure windows
%   keepfigs     - Keeps figure windows of your choice and clears the rest.
%   black        - Presets for making b/w graphs
%   vtkwrite     - Write data to file in VTK-format.
%   vline
%   hline

