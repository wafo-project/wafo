...
!      DOUBLE PRECISION,SAVE :: XSMALL    = 4.2D-16 !cut off parameter to FI
      DOUBLE PRECISION,SAVE :: XMAX    = 5.d0 !8.29287554336168D0 !cut off parameter to FI
...
! The inverse cdf of 0 is -inf, and the inverse cdf of 1 is inf.  
      ...
      if (EPSL.LT.Ak .AND. Ak.LT.amax) THEN
         ...
         IF (Ak.LT.1D-2) THEN   ! starting guess always less than 0.2 from the true value
            IF (P.GE.0.5D0) THEN 
               xk = DSQRT(-2D0*log(2D0*(1D0-P)))
            ELSE
               xk = -DSQRT(-2D0*log(2D0*P))
            ENDIF
         ELSE
     ...
     &              (Zk+P1)))
      ...
      ELSEIF (ABS(A).LE.EPSL) THEN
         IF (P>=0.5D0) THEN
            VAL = DSQRT(-2D0*log(2D0*(1.D0-P)))
         ELSE
            VAL = -DSQRT(-2D0*log(2D0*P));
         ENDIF
      ELSE  ! ABS(A) > AMAX
         VAL = FIINV(P)
      ENDIF
...
! Local variables
      ...
      DO I = 1, Ntd   
         IF ( INFI(I) .LT. 0 ) THEN 
            WRITE(string,111) INDEX1(I),COF(I,1)
 111        FORMAT (i2,' -inf inf ',10F10.5) 
         ELSE IF ( INFI(I) .EQ. 0 ) THEN 
            WRITE(string,112) INDEX1(I),B(I),COF(I,1)
 112        FORMAT (i2,' -inf ', F10.2,' ',10F10.5)
         ELSE IF ( INFI(I) .EQ. 1 ) THEN 
            WRITE(string,113) INDEX1(I),A(I),COF(I,1)
 113        FORMAT (i2,' ', F10.2 ,' inf ',10F10.5)
         ELSE 
            WRITE(string,114) INDEX1(I),A(I),B(I),COF(I,1)
 114        FORMAT (i2,' ', F10.2 ,' ', F10.2,' ',10F10.5)
         END IF
         CALL mexprintf(string) 
         DO K = 2,J
...
!
      USE GLOBALCONST
      USE GLOBALDATA, ONLY : xCutOff,EPS2,EPS
      IMPLICIT NONE
...
! Note this routine only works on the upper triangular part of R
! 
! R         = Input: Cov(X) where X=[Xt Xd Xc] is stochastic vector
!            Output: sorted Conditional Covar. matrix, i.e.,
!                    [ Cov([Xt,Xd] | Xc)    Shape N X N  (N=Ntdc=Nt+Nd+Nc) 
! index1    = permutation vector giving the indices to the variables original place.   Size  Ntdc
! 
...
!              On input: 
!               1a) the first upper triangular the Nt + Nd times Nt + Nd block contains COV([Xt,Xd]|Xc)
!                 (conditional covariance matrix for Xt and Xd given Xc)
!               2a) The upper triangular part of the Nt+Nd+Nc times Nc last block contains the
!                  cholesky matrix for Xc, i.e., 
!              On output: 
...
!               CDI(J) = STD(Xj| X1,,,Xj-1,Xc) if Xj is stochastic given X1,...Xj, Xc
!              or
!               CDI(J) = COV(Xj,Xk|X1,..,Xk-1,Xc  )/STD(Xk| X1,,,Xk-1,Xc) 
!               if Xj is determinstically determined given X1,..,Xk,Xc for some k<j.
!       
!     Subroutine to sort integration limits and determine Cholesky
!     factor.
!     
!     Note this routine works only on the upper triangular part of R
...
!     
!     Move any doubly infinite limits or any redundant of Xt to innermost positions.
!     
      DO I = N, N-INFIS+1, -1
         IF ( ((INFI(I) .GE. 0).AND. (R(I,I).GT. LTOL) )
     &        .OR. INDEX1(I).GT.Nt) THEN 
            DO J = 1,I-1
               IF ( (INFI(J) .LT. 0 .OR. (R(I,I).LE. LTOL)) 
     &              .AND. INDEX1(J).LE.Nt) THEN
                                !CALL RCSWAP( J, I,N, A, B, INFI,INDEX1,Cm, R )
         ...
         ENDIF
 10   END DO
!     Move any redundant variables of Xd to the next innermost positions. 
      DO I = N-INFIS, N-INFIS-INFISD+1, -1
         IF ( R(I,I) .GT. LTOL) THEN 
            DO J = 1,I-1
               IF ( R(I,I) .LE. LTOL) THEN
                                !CALL RCSWAP( J, I,N, A, B, INFI,INDEX1,Cm, R)
                  CALL RCSWAP( J, I, N,N, R,INDEX1,Cm, A, B, INFI)
                  GO TO 15
               ENDIF
...
!     
 40      IF ( CVDIAG.GT.TOL) THEN
            ...
            IF ( K.LT.JMIN ) THEN
                                !CALL RCSWAP( K,JMIN,N1,A,B,INFI,INDEX1,Cm,R)
               ...
               END DO
            END IF
            IF (INDEX1(K).GT.Nt) THEN
               Ndleft = Ndleft-1
            ELSE
               IF (BCVSRT.EQ..FALSE..AND.PRBMIN.EQ.gONE) THEN
                  D = gZERO
                  DO I = 1, K0-1
                     D = D + ABS(R(I,K))*xCutOff
                  END DO
                  D = D/CVDIAG
                  CALL ADJLIMITS(AMIN+D,BMIN-D,INFI(K))
               ENDIF
            ENDIF
                     
            !R(K,K0) = CVDIAG ! wrong? (old)
            R(K0,K) = CVDIAG ! correct?
            ...
            DO I = K1+1,N1              
               ...
               IF (R(I,I).GT.LTOL) THEN
                  R(I,K0) = DSQRT(R(I,I)) ! STD(Xi|X1,X2,...Xk)
               ELSE   !!IF (R(I,I) .LE. LTOL) THEN !TOL
                  R(I,K0) = MAX(DSQRT(MAX(R(I,I), gZERO)),LTOL)
                  !R(I,K0) = DSQRT(LTOL) 
                  Nullity = Nullity + 1
                  K  = K + 1
                  IF (K .LT. I) THEN
                                !CALL RCSWAP(K,I,N1,A,B,INFI,INDEX1,Cm,R)
                     CALL RCSWAP( K, I, N1,N,R,INDEX1,Cm, A, B, INFI)
                     ! SWAP conditional standarddeviations
                     DO J=1,K0
                        CALL SWAPRE(R(K,J),R(I,J))
                     END DO
                  ENDIF
            ...
            IF ( PRBMIN .GT. ZERO) THEN
               ...
               IF (INFI(K1).GE.0) THEN
                  IF (INFI(K1) .NE. 0) YL =-EXP(-HALF*(AMIN**2))/SQTWPI
                  IF (INFI(K1) .NE. 1) YU =-EXP(-HALF*(BMIN**2))/SQTWPI
               ENDIF
               Y(K0) = ( YU - YL )/PRBMIN
            ELSE
               SELECT CASE (INFI(K1))
         ...
         ELSE
            ...
            DO I = K,N1
               !TMP = R(I,K0-1)
               !R(I,K0-1) = DSQRT(MAX(R(I,I), gZERO))
               CDI(I) = R(K0-1,I)
               !A(I) = (A(I)-xCutOff*TMP)/CDI(I)
               !B(I) = (B(I)+xCutOff*TMP)/CDI(I)
               A(I) = A(I)/CDI(I)
               B(I) = B(I)/CDI(I)
...
!     Local variables
      INTEGER :: N,N1,I, J, K, L, M, II, IJ, JMIN,Ndleft
      INTEGER :: KK, K1, KK1, K0, Nullity, INFJ, FINA, FINB
      DOUBLE PRECISION :: SUMSQ, AJ, BJ, TMP,D, E,EPSL
      DOUBLE PRECISION :: AA, Ca, Pa, APJ, PRBJ
      DOUBLE PRECISION :: CVDIAG, AMIN, BMIN, PRBMIN, YL, YU
      DOUBLE PRECISION :: SQTWPI,LTOL,TOL,ONE,ZERO,HALF 
...
*    C     = upper triangular cholesky factor.Cov([Xt,Xd,Xc])
...
! local variable
      INTEGER :: J, Ntdc
      IF (PRESENT(Cm))    CALL SWAPRE( Cm(P), Cm(Q) )
      IF (PRESENT(A))     CALL SWAPRE( A(P), A(Q) )
      IF (PRESENT(B))     CALL SWAPRE( B(P), B(Q) )
      IF (PRESENT(INFIN)) CALL SWAPINT(INFIN(P),INFIN(Q))
	
      CALL SWAPINT(IND(P),IND(Q))
      
      CALL SWAPRE( C(P,P), C(Q,Q) )
      DO J = 1, P-1
         CALL SWAPRE( C(J,P), C(J,Q) )
      END DO
      DO J = P+1, Q-1
         CALL SWAPRE( C(P,J), C(J,Q) )
      END DO
      DO J = Q+1, N
         CALL SWAPRE( C(P,J), C(Q,J) )
      END DO
      Ntdc = SIZE(C,DIM=1)
      IF (N.LT.Ntdc) THEN
         DO J = Ntd+1, Ntdc
      ...
      SUBROUTINE RCSWAPOLD( P, Q, N, A, B, INFIN, IND, Cm, C )
      IMPLICIT NONE
* RCSWAP  Swaps rows and columns P and Q in situ, with P <= Q.
*
*
...
*    P, Q  = row/column number to swap P<=Q<=N
*    N     = length of A, B
*    C     = upper triangular cholesky factor.
*    A,B   = lower and upper integration limit, respectively.
*    INFIN = if INFIN(I) < 0, Ith limits are (-infinity, infinity);
*            if INFIN(I) = 0, Ith limits are (-infinity, B(I)];
*            if INFIN(I) = 1, Ith limits are [A(I), infinity);
...
*
      DOUBLE PRECISION, DIMENSION(:),INTENT(inout)   :: A, B, Cm
	DOUBLE PRECISION, DIMENSION(:,:),INTENT(inout) :: C
...
! variables set in initfun and used in mvnfun:
      INTEGER, PRIVATE :: I0,NdleftN0,IJ0
...
! INDEX1 - if INDEX1(I)>Nt then variable No. I is one of the Xd variables otherwise it is one of Xt
     ...
     &        Y(IK)*CDIXd(NdleftN+1:NdleftO)
      ...
      DO I = I0+1, N+1
         ...
         IF ( INFI(I) .NE. 0 ) THEN
            IF ( INFA .EQ. 1 ) THEN
               AI = MAX( AI, A(I) - TMP - xCutOff*BIG(I,IK)) 
            ELSE
               AI = A(I)-TMP 
         ...
         IF ( INFI(I) .NE. 1 ) THEN
            IF ( INFB .EQ. 1 ) THEN
               BI = MIN( BI, B(I) - TMP + xCutOff*BIG(I,IK))	
            ELSE
               BI = B(I)-TMP 
...
! Variables set in initfun and used in mvnfun
      INTEGER, PRIVATE,          SAVE 	:: I0,NdleftN0,IJ0
...
!     Print *,'rindd ndim1',Ndim1
 ...
 100     IF (INDEX1(I).GT.Nt) THEN ! Save the mean for Xd
         ...
         IF (I.EQ.N+1.OR.BIG(2,I+1).GT.ZERO) THEN
            CALL MVNLIMITS( AI,BI,2*INFA+INFB-1,AP1, PRB1,AQ1)
...
!     ENDIF
 ...
 100     IF (INDEX1(I).GT.Nt) THEN ! Save the mean of xd and Covariance diagonal element 
         ...
         IF ( I .EQ. N+1 .OR. BIG(IK+1,I+1) .GT.ZERO) THEN   
            CALL MVNLIMITS(AI,BI,2*INFA+INFB-1,API,PRBI,AQI)            
