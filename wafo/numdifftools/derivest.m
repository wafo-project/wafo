...
% 
% CALL: [der,errest] = derivest(fun,x0)  % first derivative
%       [der,errest] = derivest(fun,x0,opts1,prop1,val1,prop2,val2,...)
%
% Arguments: (output)
...
%
% Arguments (input)
%  fun = inline-, anonymous-, or an m-file-function to differentiate. 
%        IF fun is not vectorized, then you MUST set 'vectorized' property
%         to 'no', so that derivest will call your function sequentially instead.
%  x0  = scalar, vector, or array of points at which to differentiate fun.
% opts = struct options with fieldnames
%  .DerivativeOrder : Integer between 1 and 4 defining derivative order.
%                     (DEFAULT 1 (first derivative))
%  .MethodOrder     : Order of the basic method used for the estimation.
%        For 'central' methods, it must be an integer from the set [2,4].
%        For 'forward' or 'backward' difference methods, it must be an 
%        integer from the set [1,2,3,4]. (Default 4 (a second order method))
%  .Style           : Style of the basic method used for the estimation. 
%                     Valid options are: 'central', 'forward' or
%                     'backwards'.     (DEFAULT 'Central')
%  .RombergTerms    : Number of Romberg terms used in the extrapolation.
%        Must be a integer from the set [0,1,2,3].  DEFAULT: 2 (Two Romberg
%        terms) Note: 0 disables the Romberg step completely.
%  .FixedStep'      : If specified, it will define the maximum excursion 
%        from x0 that is used and prevent the adaptive logic from working.
%        This will be considerably faster, but not necessarily
%        as accurate as allowing the adaptive logic to run. 
%        (DEFAULT: [] (unspecified)
%  .Vectorized      : 'yes' if your function is vectorized (Default).
%                     'no'. This will cause derivest to loop over the 
%                     successive function calls.
%  .MaxStep         : Maximum excursion from x0 that is allowed, as a
%                     multiple of x0. (DEFAULT 100)
...
% provided in fun. Fun is assumed to return a result of the same shape as
% its input x0. It will not differentiate a function provided as data. 
% Use gradient for that purpose, or differentiate a spline model.
%     
% The methods used by DERIVEST are finite difference approximations of 
% various orders, coupled with a generalized (multiple term) Romberg
% extrapolation. This also yields the error estimate provided. DERIVEST 
% uses a semi-adaptive scheme to provide the best estimate that it can by
% its automatic choice of a differencing interval.
%
% See the document DERIVEST.pdf for more explanation of the
% algorithms behind the parameters of DERIVEST.
%
% Note on MethodOrder: higher order methods will generally be more accurate,
%         but may also suffere more from numerical problems. First order
%         methods would usually not be recommended.
% Note on Style: Central difference methods are usually the most accurate,
...
% -replace code with parseoptions -> now possible to supply option structs.
